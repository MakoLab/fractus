package com.makolab.components.inputComponents
{
	...
	public class PackageEditor extends VBox implements IDataObjectComponent, IFormBuilderComponent
	{
		/**
		...
		protected function createPopUp():UIComponent
		{
			...
			for (var s:String in attributes) 
				if (attributes[s].metadata.disabled.length() == 0 || attributes[s].metadata.disabled == 0) 
					{visibleAttributes.push(attributes[s]);trace(s,"  attributes[s]:  ",attributes[s]);}
		...
		protected function getAttributeNode():XML
		{
			...
			trace("res:",res," l:", res.length()," 0:", res[0]);
		...
		{  
			...
			for (var j:int = 0; j < n; j++)	// current attribute index
			{
				...
				if (attributeType)	// if the type belongs to this editor
				{
					...
					if (editors[i].label) editors[i].label.text = attributeType.label;
		...
		protected function handleDeleteClick(event:MouseEvent):void
		{
			for (var i:String in editors) if (editors[i].deleteBtn == event.target)
			{
				...
				Alert.show(LanguageManager.getInstance().labels.alert.realyDelete + " \"" + editors[i].attributeType.label + "\"?", LanguageManager.getInstance().labels.alert.deletingAttribute, Alert.YES | Alert.NO, this, handleConfirmDialogClose);
				return;
			}
		}
