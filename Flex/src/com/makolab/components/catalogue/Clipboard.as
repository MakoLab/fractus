package com.makolab.components.catalogue
{
	...
	public class Clipboard extends EventDispatcher
	{
		 ...
		 * Constuctor.
		 */
		public function Clipboard(enforcer:SingletonEnforcer)
		{
			// hard text
			if (enforcer == null) throw new Error("You Can Only Have One Clipboard Instance");
		}
		
		private static var instance:Clipboard;
		
		public static function getInstance():Clipboard
		{
			if (!instance)
			{
				instance = new Clipboard(new SingletonEnforcer);
				...
				ModelLocator.getInstance().configManager.requestList(["cart","cart.content"],instance.configurationLoaded);
			}
			return instance;
		}
		...
		{
			var config:XML = ModelLocator.getInstance().configManager.getXML("cart");
			if (config != null)
			{
				if (config.columns){
