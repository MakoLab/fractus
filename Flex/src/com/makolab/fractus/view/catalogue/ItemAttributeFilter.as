package com.makolab.fractus.view.catalogue
{
	...
	public class ItemAttributeFilter extends FormItem implements ICatalogueFilter
	{
		...
		public function ItemAttributeFilter()
		{
			super();
		}
		...
		override protected function createChildren():void
		{
			...
			trace(attributeName,LanguageManager.getInstance().currentLanguage,DictionaryManager.getInstance().dictionaries.itemAttributes.(name == attributeName).label.(@lang == LanguageManager.getInstance().currentLanguage)[0]);
				
			this.label = DictionaryManager.getInstance().dictionaries.itemAttributes.(name == attributeName).label.(@lang == LanguageManager.getInstance().currentLanguage)[0];
		}
				...
				if (combo) 
					...
					if(type=="boolean")
					{
						if(combo.selectedItem.name.toString()!="-1")
							condition = Tools.replaceParameters(
								/*(type == 'partial' ? QUERY_TEMPLATE_BOOL : QUERY_TEMPLATE)*/QUERY_TEMPLATE_BOOL,
								{ 'attributeName' : this.attributeName, 'attributeValue' : combo.selectedItem.name.toString() }
							);
