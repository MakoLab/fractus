/*
name=[reports].[p_getSalesByContractor]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
GxuRHTcakrCZgAAVVeYtjA==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getSalesByContractor]') AND type in (N'P', N'PC'))
DROP PROCEDURE [reports].[p_getSalesByContractor]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getSalesByContractor]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE [reports].[p_getSalesByContractor]
@xmlVar XML
AS
BEGIN
        DECLARE 
        ...
        SELECT  @opakowanie = ''	DECLARE @return XML '',
                @select = ''SELECT @return = (  SELECT * FROM (
						SELECT id, contractorCode, contractorName, quantity,cost, netValue, grossValue, (netValue - cost) profit, (netValue - cost) / NULLIF(netValue,0) profitMargin
						FROM (
						SELECT c.id, ISNULL(NULLIF(RTRIM(c.code) ,''''''''),''''-'''') contractorCode, ISNULL(NULLIF(RTRIM(c.shortName) ,'''''''') ,''''-'''') contractorName 
						...
						--,  (sum((l.netValue * CommercialDocumentHeader.exchangeRate)/CommercialDocumentHeader.exchangeScale) - sum(ISNULL(cv.value,0))) / nullif(sum((l.netValue * CommercialDocumentHeader.exchangeRate)/CommercialDocumentHeader.exchangeScale), 0) profitMargin
						'',
                @from = ''	FROM document.CommercialDocumentHeader  WITH(NOLOCK) 
							LEFT JOIN contractor.Contractor c WITH(NOLOCK)ON c.id = CommercialDocumentHeader.contractorId  
		...
		SELECT @exec	= @opakowanie + @select + @from + ISNULL( '' WHERE '' + @where ,'''') + ISNULL(@condition ,'''') + '' GROUP BY c.id, NULLIF(RTRIM(c.code) ,''''''''), NULLIF(RTRIM(c.shortName) ,'''''''') )x  ) line ORDER BY contractorCode, contractorName FOR XML AUTO);
