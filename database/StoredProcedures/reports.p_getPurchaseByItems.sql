/*
name=[reports].[p_getPurchaseByItems]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
0cWG0HoM2ylXpyD9rNamJA==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getPurchaseByItems]') AND type in (N'P', N'PC'))
DROP PROCEDURE [reports].[p_getPurchaseByItems]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getPurchaseByItems]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [reports].[p_getPurchaseByItems]
@xmlVar XML
AS
BEGIN
        DECLARE 
		...
		SELECT @where = ''  l.commercialDirection <> 0 '' + @condition
        ...
        SELECT  @opakowanie = ''	DECLARE @return XML '',
                @select = ''SELECT @return = (  SELECT * FROM (SELECT i.id ,i.name itemName , i.code itemCode, sum(l.quantity) quantity , sum(ISNULL(cv.value,0)) cost , sum((l.netValue * CommercialDocumentHeader.exchangeRate)/CommercialDocumentHeader.exchangeScale) netValue , sum(( l.grossValue * CommercialDocumentHeader.exchangeRate)/CommercialDocumentHeader.exchangeScale) grossValue ,  (sum((l.netValue * CommercialDocumentHeader.exchangeRate)/CommercialDocumentHeader.exchangeScale) - sum(ISNULL(cv.value,0))) profit ,  (sum((l.netValue * CommercialDocumentHeader.exchangeRate)/CommercialDocumentHeader.exchangeScale) - sum(ISNULL(cv.value,0))) / NULLIF(sum( (l.grossValue * CommercialDocumentHeader.exchangeRate)/CommercialDocumentHeader.exchangeScale),0) profitMargin, (SELECT textValue FROM item.ItemAttrValue va WHERE va.itemId = i.id AND va.itemFieldId = (select id from dictionary.ItemField where name like ''''Attribute_Manufacturer'''')) manufacturer'',
                @from = ''	FROM document.CommercialDocumentHeader WITH(NOLOCK)  
							...
							JOIN document.CommercialDocumentLine l WITH(NOLOCK) ON CommercialDocumentHeader.id = l.commercialDocumentHeaderId 
		...
		SELECT @exec	= @opakowanie + @select + @from + ISNULL( '' WHERE '' + @where ,'''') + '' GROUP BY i.id ,i.name ,  i.code ) line  ORDER BY itemName FOR XML AUTO);
