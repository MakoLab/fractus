/*
name=[reports].[p_getSalesByContractorGroup]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
dr+PpYsQ53tCycVP2a9ilQ==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getSalesByContractorGroup]') AND type in (N'P', N'PC'))
DROP PROCEDURE [reports].[p_getSalesByContractorGroup]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getSalesByContractorGroup]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE [reports].[p_getSalesByContractorGroup]
@xmlVar XML
AS
BEGIN
        DECLARE 
        ...
        SELECT  @opakowanie = ''	DECLARE @return XML '',
                @select = ''SELECT @return = (  SELECT * FROM (
						SELECT contractorGroupId, quantity,cost, netValue, grossValue, (netValue - cost) profit, (netValue - cost) / NULLIF(netValue,0) profitMargin
						FROM (
							SELECT c.contractorGroupId contractorGroupId , sum(l.quantity) quantity , 
							sum(ABS(ISNULL(cv.value,0)) * SIGN(l.quantity )) cost 
							, sum(l.netValue * CommercialDocumentHeader.exchangeRate) netValue 
							, sum( l.grossValue * CommercialDocumentHeader.exchangeRate) grossValue 
							--,  (sum(l.netValue * CommercialDocumentHeader.exchangeRate) - sum(ISNULL(cv.value,0))) profit  
							--, (sum(l.netValue * CommercialDocumentHeader.exchangeRate) - sum(ISNULL(cv.value,0))) / NULLIF(sum( l.netValue * CommercialDocumentHeader.exchangeRate),0) profitMargin
							'',
                @from = ''	FROM document.CommercialDocumentHeader  WITH(NOLOCK) 
							...
							LEFT JOIN contractor.ContractorGroupMembership c WITH(NOLOCK) ON c.contractorId = CommercialDocumentHeader.contractorId
							...
							LEFT JOIN contractor.Contractor cc ON  CommercialDocumentHeader.contractorId = cc.id ''
        ...
        IF NULLIF((SELECT @contractorGroups.value(''.'',''varchar(8000)'') ),'''') IS NOT NULL
                    SELECT  @where = ISNULL( @where + CASE WHEN NULLIF(@includeUnassignedContractors, '''') IS NOT NULL THEN '' OR '' ELSE '' AND '' END, '''') +
                             '' CommercialDocumentHeader.contractorId IN (SELECT CGM2.contractorId FROM contractor.ContractorGroupMembership CGM2 WITH(NOLOCK) WHERE CGM2.contractorGroupId in ('''''' + REPLACE(CAST(@itemGroups.query(''.'')as varchar(max)) ,'','','''''','''''') + '''''') ) ''
		...
		SELECT @exec	= @opakowanie + @select + @from + ISNULL( '' WHERE '' + @where ,'''') + ISNULL(@condition ,'''') + '' GROUP BY c.contractorGroupId ) x ) line ORDER BY contractorGroupId FOR XML AUTO);
