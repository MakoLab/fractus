/*
name=[reports].[p_getSalesByMonth]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
cpBMCyZFFpduoVO5g8XNRQ==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getSalesByMonth]') AND type in (N'P', N'PC'))
DROP PROCEDURE [reports].[p_getSalesByMonth]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getSalesByMonth]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE [reports].[p_getSalesByMonth]
@xmlVar XML
AS
BEGIN
        DECLARE 
        ...
        SELECT  @opakowanie = ''	DECLARE @return XML '',
                @select = ''SELECT @return = (  SELECT * FROM (
						SELECT month, quantity,cost, netValue, grossValue, (netValue - cost) profit, (netValue - cost) / NULLIF(netValue,0) profitMargin
						FROM (
							SELECT left(convert(varchar(10), CommercialDocumentHeader.eventDate, 21),4) + ''''-'''' + substring(convert(varchar(10), CommercialDocumentHeader.eventDate, 21),6,2) month 
		...
		SELECT @exec	= @opakowanie + @select + @from + ISNULL( '' WHERE '' + @where ,'''') + ISNULL(@condition ,'''') + '' GROUP BY  left(convert(varchar(10), CommercialDocumentHeader.eventDate, 21),4) + ''''-'''' + substring(convert(varchar(10), CommercialDocumentHeader.eventDate, 21),6,2)   ) x ) line  ORDER BY month DESC FOR XML AUTO);
