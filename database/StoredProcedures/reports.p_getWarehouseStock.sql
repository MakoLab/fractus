/*
name=[reports].[p_getWarehouseStock]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
nsj05k9SpGglGiUofpLLxw==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getWarehouseStock]') AND type in (N'P', N'PC'))
DROP PROCEDURE [reports].[p_getWarehouseStock]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getWarehouseStock]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE [reports].[p_getWarehouseStock]
@xmlVar XML
AS
BEGIN
        DECLARE 
        ...
        SELECT  @opakowanie = ''	DECLARE @return XML '',
                @select = ''SELECT @return = (  
								SELECT * FROM (	
									...
									'' + CASE WHEN @dateTo IS NULL 
											THEN ''ISNULL(SUM(ws.quantity),0) quantity, 0 cost, ''
											ELSE ''
										
									sum(l.quantity * direction) quantity ,
									 ISNULL(sum(ISNULL(ABS(l.value) * SIGN(l.quantity * l.direction),0)) , 0 ) cost,''
									 END + '' 
									(SELECT top 1 textValue FROM item.ItemAttrValue va WHERE va.itemId = i.id AND va.itemFieldId = (select id from dictionary.ItemField where name like ''''Attribute_Manufacturer'''')) manufacturer
			    ...
			    @from = ''			FROM item.Item i WITH(NOLOCK) 
										LEFT JOIN (SELECT itemId, sum(ISNULL(quantity,0)) quantity FROM document.WarehouseStock WITH(NOLOCK) WHERE warehouseId in (SELECT id FROM dictionary.Warehouse where branchId in (SELECT id FROM dictionary.Branch WHERE databaseId = '''''' + @dbId  + ''''''))  GROUP BY itemId ) ws  ON i.id = ws.itemId 
...
--				ELSE
				...
				IF @field_name = ''zeroStock''
					IF @field_value = ''1''
						BEGIN 
							SELECT	@having =  '' HAVING ISNULL(SUM(ws.quantity),0) = 0'' 
						END
						ELSE
						BEGIN
							SELECT	@having =  '' HAVING SUM(l.quantity * direction) <>  0  ''
						END
				ELSE
...
eby nie pokazywało pozycji o zerowej ilości i niezerowym koszcie, które wynikaja z błnych kosztów na przychodach/rozchodach */ 
		...
		SELECT @exec = @opakowanie + @select + @from + ISNULL( char(10) + '' WHERE '' + @where ,'''') + '' GROUP BY i.id ,i.name ,  i.code  
			''  + CASE WHEN @dateTo IS NULL 
					THEN  ISNULL(@having, '' HAVING SUM(ws.quantity) <> 0 '')
					ELSE  ISNULL(@having, '' HAVING sum(l.quantity * direction) <> 0 '') 
				END + ''
		) line  FOR XML AUTO);
