/*
name=[custom].[p_updateItemDictionary]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
s2hDC0QmK0tXvCVzDAmSeA==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[custom].[p_updateItemDictionary]') AND type in (N'P', N'PC'))
DROP PROCEDURE [custom].[p_updateItemDictionary]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[custom].[p_updateItemDictionary]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE [custom].[p_updateItemDictionary]
