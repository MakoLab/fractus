/*
name=[reports].[p_getSalesByItems]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
clQMeh58crVfA2BMbao/Lw==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getSalesByItems]') AND type in (N'P', N'PC'))
DROP PROCEDURE [reports].[p_getSalesByItems]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getSalesByItems]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE [reports].[p_getSalesByItems]
@xmlVar XML
AS
BEGIN

/*
reports.p_getSalesByItems
''<searchParams type="CommercialDocument">
  ...
  <query>Głogów</query>
  <dateFrom>2011-11-16</dateFrom>
  <dateTo>2011-11-16T23:59:59.997</dateTo>
</searchParams>''
*/
        ...
        SELECT  @opakowanie = ''	DECLARE @return XML '',
                @select = ''SELECT @return = (  SELECT * FROM (
							...
							SELECT i.id ,i.name itemName , i.code itemCode, -(sum(l.quantity * l.commercialDirection)) quantity , 
									...
									(SELECT textValue FROM item.ItemAttrValue va WHERE va.itemId = i.id AND va.itemFieldId = (select id from dictionary.ItemField where name like ''''Attribute_Manufacturer'''')) manufacturer'',
...
--SELECT SUM( CASE WHEN ISNULL(value,0) > 0 THEN ISNULL(value,0) ELSE 0 END) valueP,
--		SUM( CASE WHEN ISNULL(value,0) < 0 THEN ISNULL(value,0) ELSE 0 END) valueN, 
--		commercialDocumentLineId 
--FROM document.CommercialWarehouseValuation WITH(NOLOCK) WHERE commercialDocumentLineId IS NOT NULL  GROUP BY commercialDocumentLineId 
		...
		SELECT @exec	= @opakowanie + @select + @from + ISNULL( '' WHERE '' + @where ,'''')  + ISNULL(@condition ,'''') +  '' GROUP BY  i.id ,i.name  , i.code   ) line ORDER BY itemName FOR XML AUTO);
