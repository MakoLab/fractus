/*
name=[reports].[p_getPurchaseByQuarter]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
wKX52W93MPjiCO8eEnUF7w==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getPurchaseByQuarter]') AND type in (N'P', N'PC'))
DROP PROCEDURE [reports].[p_getPurchaseByQuarter]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getPurchaseByQuarter]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [reports].[p_getPurchaseByQuarter]
@xmlVar XML
AS
BEGIN
        DECLARE 
		...
		SELECT @exec	= @opakowanie + @select + @from + ISNULL( '' WHERE '' + @where ,'''') + '' GROUP BY  left(convert(varchar(10), CommercialDocumentHeader.eventDate, 21),4) + cast(datepart(qq,CommercialDocumentHeader.eventDate) as varchar(50))   ) line ORDER BY quarter  FOR XML AUTO);
