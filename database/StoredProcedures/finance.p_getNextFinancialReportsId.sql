/*
name=[finance].[p_getNextFinancialReportsId]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
GE7JE2rqenaQFIM3DozqUg==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[finance].[p_getNextFinancialReportsId]') AND type in (N'P', N'PC'))
DROP PROCEDURE [finance].[p_getNextFinancialReportsId]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[finance].[p_getNextFinancialReportsId]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [finance].[p_getNextFinancialReportsId]
@xmlVar XML
AS
BEGIN
	...
	SELECT @x = (
		...
		FROM finance.FinancialReport r WITH(NOLOCK) 
			...
			LEFT JOIN document.FinancialDocumentHeader WITH(NOLOCK) ON FinancialDocumentHeader.financialReportId = r.id
