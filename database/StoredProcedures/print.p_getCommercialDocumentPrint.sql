/*
name=[print].[p_getCommercialDocumentPrint]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
se1OPkFDwblbioozznlMQQ==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[print].[p_getCommercialDocumentPrint]') AND type in (N'P', N'PC'))
DROP PROCEDURE [print].[p_getCommercialDocumentPrint]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[print].[p_getCommercialDocumentPrint]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE [print].[p_getCommercialDocumentPrint] 
...
IF EXISTS( SELECT * FROM document.DocumentRelation WHERE secondCommercialDocumentHeaderId = @documentHeaderId)
	...
	WHERE dr.secondCommercialDocumentHeaderId = @documentHeaderId AND dr2.relationType IN (9, 12) 
        ...
        SELECT  (
			SELECT ( 
				  SELECT
						...
						CDL.exchangeScale,
						CDL.exchangeDate,
						CDL.xmlConstantData.query(''//constant'') xmlConstantData
						,
						(SELECT (
						...
						(SELECT (SELECT vc.xmlValue.query(''payment/*'')
								FROM [finance].[v_paymentPrintData] vc WITH (NOLOCK)
								WHERE vc.commercialDocumentHeaderId = CDL.id FOR XML PATH(''payment''), TYPE)
				...
				( SELECT
						(SELECT 
							  ...
							  vt.[version],
							  vt.[order],
							  vt.[netValue] * h.exchangeRate [netValuePLN],
							  vt.[grossValue] * h.exchangeRate [grossValuePLN],
							  vt.[vatValue] * h.exchangeRate [vatValuePLN]
						 FROM document.CommercialDocumentHeader h  WITH(NOLOCK) JOIN  document.CommercialDocumentVatTable vt WITH(NOLOCK) ON h.id =  vt.[commercialDocumentHeaderId]
