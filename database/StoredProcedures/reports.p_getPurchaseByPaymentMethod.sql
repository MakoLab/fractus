/*
name=[reports].[p_getPurchaseByPaymentMethod]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
W8PGdOUAcDJh6E5+KSstKg==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getPurchaseByPaymentMethod]') AND type in (N'P', N'PC'))
DROP PROCEDURE [reports].[p_getPurchaseByPaymentMethod]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getPurchaseByPaymentMethod]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [reports].[p_getPurchaseByPaymentMethod]
@xmlVar XML
AS
BEGIN
        DECLARE 
		...
		SELECT	@replaceConf_item = c.xmlValue.value(''(root/indexing/object[@name="item"]/replaceConfiguration)[1]'', ''varchar(8000)'')
        ...
        SELECT  @opakowanie = ''	DECLARE @return XML '',
                @select = ''SELECT @return = (  SELECT * FROM (SELECT pm.paymentMethodId , sum(l.quantity) quantity , sum(ISNULL(cv.value,0)) cost , sum((l.netValue * CommercialDocumentHeader.exchangeRate)/CommercialDocumentHeader.exchangeScale) netValue , sum( (l.grossValue * CommercialDocumentHeader.exchangeRate)/CommercialDocumentHeader.exchangeScale) grossValue ,  (sum((l.netValue * CommercialDocumentHeader.exchangeRate)/CommercialDocumentHeader.exchangeScale) - sum(ISNULL(cv.value,0))) profit ,  (sum((l.netValue * CommercialDocumentHeader.exchangeRate)/CommercialDocumentHeader.exchangeScale) - sum(ISNULL(cv.value,0))) / sum( (l.grossValue * CommercialDocumentHeader.exchangeRate)/CommercialDocumentHeader.exchangeScale) profitMargin '',
                @from = ''	FROM finance.Payment pm WITH(NOLOCK) 
							JOIN document.CommercialDocumentHeader WITH(NOLOCK) ON pm.commercialDocumentHeaderId = CommercialDocumentHeader.id AND CommercialDocumentHeader.status >= 40
							JOIN dictionary.DocumentType dt WITH(NOLOCK) ON CommercialDocumentHeader.documentTypeId = dt.id AND documentCategory IN (2,6)
		...
		WHILE @i <= @filter_count
			BEGIN
				...
				IF @field_name = ''paymentMethodId''
						SELECT	@where = ISNULL( @where + '' AND '','' '' )  + ''( pm.paymentMethodId = '''''' + REPLACE(@field_value,'','','''''' OR  pm.paymentMethodId = '''''') + '''''' ) ''
				ELSE
		...
		SELECT @exec	= @opakowanie + @select + @from + ISNULL( '' WHERE '' + @where ,'''') + '' GROUP BY  pm.paymentMethodId   ) line ORDER BY paymentMethodId FOR XML AUTO);
