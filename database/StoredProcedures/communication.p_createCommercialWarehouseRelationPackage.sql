/*
name=[communication].[p_createCommercialWarehouseRelationPackage]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
2CAmCt224zSI9KMsdahreA==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[communication].[p_createCommercialWarehouseRelationPackage]') AND type in (N'P', N'PC'))
DROP PROCEDURE [communication].[p_createCommercialWarehouseRelationPackage]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[communication].[p_createCommercialWarehouseRelationPackage]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [communication].[p_createCommercialWarehouseRelationPackage]
@xmlVar XML
AS
BEGIN
        ...
        SELECT  @snap = ( SELECT    ( 
									SELECT (
									  SELECT    x.value(''@previousVersion'',''char(36)'') AS ''@previousVersion'',
												...
												ISNULL(version, x.value(''@previousVersion'',''char(36)'') )  ''version''
                                      FROM      @xmlVar.nodes(''root/entry'') AS a ( x )
                                                LEFT JOIN document.CommercialWarehouseRelation i ON i.id = x.value(''@id'', ''char(36)'')
