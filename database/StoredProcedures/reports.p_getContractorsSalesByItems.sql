/*
name=[reports].[p_getContractorsSalesByItems]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
+++plLv5Pp1V4/t+2n2y6g==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getContractorsSalesByItems]') AND type in (N'P', N'PC'))
DROP PROCEDURE [reports].[p_getContractorsSalesByItems]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getContractorsSalesByItems]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [reports].[p_getContractorsSalesByItems]
@xmlVar XML
AS
BEGIN
...
*/
        ...
        SELECT  
		 	...
		 	SELECT  
				(
					SELECT i.name as  ''@itemName'' , i.code AS ''@itemCode'', 
