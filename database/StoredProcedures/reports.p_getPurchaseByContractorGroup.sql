/*
name=[reports].[p_getPurchaseByContractorGroup]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
+b60nQyvenjZdY8iGAzCUw==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getPurchaseByContractorGroup]') AND type in (N'P', N'PC'))
DROP PROCEDURE [reports].[p_getPurchaseByContractorGroup]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getPurchaseByContractorGroup]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE [reports].[p_getPurchaseByContractorGroup]
@xmlVar XML
AS
BEGIN
        DECLARE 
		...
		SELECT @exec	= @opakowanie + @select + @from + ISNULL( '' WHERE '' + @where ,'''') + '' GROUP BY c.contractorGroupId ) line ORDER BY contractorGroupId FOR XML AUTO);
