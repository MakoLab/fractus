/*
name=[reports].[p_getPurchaseByContractor]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
AHpy478w4GA13MTBlevpNQ==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getPurchaseByContractor]') AND type in (N'P', N'PC'))
DROP PROCEDURE [reports].[p_getPurchaseByContractor]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getPurchaseByContractor]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE [reports].[p_getPurchaseByContractor]
@xmlVar XML
AS
BEGIN
        DECLARE 
        ...
        SELECT  @opakowanie = ''	DECLARE @return XML '',
                @select = ''SELECT @return = (  SELECT * FROM (SELECT c.id, c.code contractorCode, c.shortName contractorName , sum(l.quantity) quantity, sum(ISNULL(cv.value,0)) cost , sum((l.netValue  * CommercialDocumentHeader.exchangeRate)/CommercialDocumentHeader.exchangeScale) netValue , sum( (l.grossValue  * CommercialDocumentHeader.exchangeRate)/CommercialDocumentHeader.exchangeScale) grossValue ,  (sum((l.netValue  * CommercialDocumentHeader.exchangeRate)/CommercialDocumentHeader.exchangeScale) - sum(ISNULL(cv.value,0))) profit ,  (sum((l.netValue  * CommercialDocumentHeader.exchangeRate)/CommercialDocumentHeader.exchangeScale) - sum(ISNULL(cv.value,0))) / NULLIF(sum( (l.grossValue * CommercialDocumentHeader.exchangeRate)/CommercialDocumentHeader.exchangeScale ),0) profitMargin'',
                @from = ''	FROM contractor.Contractor c WITH(NOLOCK)
							JOIN document.CommercialDocumentHeader  WITH(NOLOCK) ON c.id = CommercialDocumentHeader.contractorId AND CommercialDocumentHeader.status >= 40
		...
		SELECT @exec	= @opakowanie + @select + @from + ISNULL( '' WHERE '' + @where ,'''') + '' GROUP BY c.id, c.code, c.shortName  ) line ORDER BY contractorCode FOR XML AUTO);
