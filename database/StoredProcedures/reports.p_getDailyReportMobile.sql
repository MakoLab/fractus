/*
name=[reports].[p_getDailyReportMobile]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
YJHTwDO6B6KQscKjrErA/g==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getDailyReportMobile]') AND type in (N'P', N'PC'))
DROP PROCEDURE [reports].[p_getDailyReportMobile]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getDailyReportMobile]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [reports].[p_getDailyReportMobile]
AS
------------ procedura dla zestawień na urządzenia mobilne ---------

DECLARE @branches TABLE (id uniqueidentifier, symbol varchar(100))

DECLARE @profileId uniqueidentifier, @profileIdAttribute uniqueidentifier, @date datetime
SELECT
	@date = getdate(),
	@profileId = ''D246DD19-BD9A-4B7C-B619-651ED1354312'',
	@profileIdAttribute = id
FROM dictionary.DocumentField
WHERE name = ''Attribute_DocumentIssueProfileId''

-- SET @date = ''2011-01-21''	-- do testowania wyników na dany dzień inny niż dzisiejszy

INSERT INTO @branches (id, symbol)
SELECT id, symbol FROM dictionary.Branch
UNION SELECT @profileId, ''PS''	-- dodanie oddziału "wirtualnego"
	...
	FROM @branches b
		LEFT JOIN ( --sprzedaz bez zamowien
				SELECT  (sum(l.netValue )) netValue, 
						...
						CASE WHEN dav.textValue = @profileId THEN @profileId ELSE h.branchId END AS branchId
				FROM document.CommercialDocumentHeader h WITH(NOLOCK) 
					...
					LEFT JOIN document.DocumentAttrValue dav ON dav.commercialDocumentHeaderId = h.id AND dav.documentFieldId = @profileIdAttribute
				WHERE dt.documentCategory  IN (0,5)
					...
					AND h.status >= 40
					AND  [dbo].[f_reportsSalesDateSelector]( h.issueDate, h.eventDate, dt.documentCategory , dt.symbol ) >= CONVERT(varchar(10),@date,120) AND  [dbo].[f_reportsSalesDateSelector]( h.issueDate, h.eventDate, dt.documentCategory , dt.symbol ) < CONVERT(varchar(10),DATEADD(d,1,@date),120)
				GROUP BY CASE WHEN dav.textValue = @profileId THEN @profileId ELSE h.branchId END
					
			) s ON b.id = s.branchId
		LEFT JOIN (	--stan kasy	
					 SELECT  sum(ISNULL(p.amount ,0) * ISNULL(p.direction,0)) cashReportBalance,
							CASE WHEN re.symbol = ''KASA_PS'' THEN @profileId ELSE re.branchId END AS branchId
					...
					GROUP BY CASE WHEN re.symbol = ''KASA_PS'' THEN @profileId ELSE re.branchId END
		...
		LEFT JOIN ( --miesięczna sprzedaż
				...
				WHERE dt.documentCategory  IN (0,5)
					...
					AND h.status >= 40
					AND ISNULL( xmlOptions.value(''(/root/commercialDocument/@isPrepaymentInvoice)[1]'',''char(6)''),'''') <> ''true''
					AND  YEAR([dbo].[f_reportsSalesDateSelector]( h.issueDate, h.eventDate, dt.documentCategory , dt.symbol )) = year(@date) AND MONTH([dbo].[f_reportsSalesDateSelector]( h.issueDate, h.eventDate, dt.documentCategory , dt.symbol )) = MONTH(@date)
				GROUP BY CASE WHEN dav.textValue = @profileId THEN @profileId ELSE h.branchId END
			) xx ON b.id = xx.branchId
		WHERE symbol NOT IN (''CE'', ''GR'', ''JN'', ''JB'')
		ORDER BY symbol ASC
