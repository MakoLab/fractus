/*
name=[reports].[p_getSalesByPaymentMethod]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
JWjcs+ZXyMLI3Fo8DownEQ==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getSalesByPaymentMethod]') AND type in (N'P', N'PC'))
DROP PROCEDURE [reports].[p_getSalesByPaymentMethod]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getSalesByPaymentMethod]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE [reports].[p_getSalesByPaymentMethod]
@xmlVar XML
AS
BEGIN
        DECLARE 
        ...
        SELECT  @opakowanie = ''	DECLARE @return XML '',
                ...
                SELECT paymentMethodId, quantity,cost, netValue, grossValue, (netValue - cost) profit, (netValue - cost) / NULLIF(netValue,0) profitMargin
						FROM (
						SELECT pm.paymentMethodId , sum(l.quantity) quantity 
							...
							--,  (sum((l.netValue * CommercialDocumentHeader.exchangeRate)/CommercialDocumentHeader.exchangeScale) - sum(ISNULL(cv.value,0))) profit 
							--, (sum(l.netValue * CommercialDocumentHeader.exchangeRate) - sum(ISNULL(cv.value,0))) / NULLIF(sum( (l.netValue * CommercialDocumentHeader.exchangeRate)/CommercialDocumentHeader.exchangeScale ),0) profitMargin
		...
		SELECT @exec	= @opakowanie + @select + @from + ISNULL( '' WHERE '' + @where ,'''') + ISNULL(@condition ,'''') + '' GROUP BY  pm.paymentMethodId ) x  ) line ORDER BY paymentMethodId FOR XML AUTO);
