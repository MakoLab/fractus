/*
name=[document].[p_getSalesOrderSettledAmount2]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
1Pp7yPF+B0ry/l/BTCiVBg==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[document].[p_getSalesOrderSettledAmount2]') AND type in (N'P', N'PC'))
DROP PROCEDURE [document].[p_getSalesOrderSettledAmount2]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[document].[p_getSalesOrderSettledAmount2]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE  PROCEDURE document.p_getSalesOrderSettledAmount2 -- ''CAF1E3BB-963D-47F1-95A7-2D7B02B3A2CB''
--declare
...
SELECT @xml = (
	SELECT (
		SELECT (
			SELECT vt.vatRateId ''@id'',SUM(vt.netValue) ''@netValue'',SUM(vt.grossValue) ''@grossValue''
