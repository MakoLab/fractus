/*
name=[reports].[p_getSalesTypeByIssuingPerson]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
awaK8Lj1NR/ShSD2lhuhSQ==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getSalesTypeByIssuingPerson]') AND type in (N'P', N'PC'))
DROP PROCEDURE [reports].[p_getSalesTypeByIssuingPerson]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getSalesTypeByIssuingPerson]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [reports].[p_getSalesTypeByIssuingPerson]
@xmlVar XML
AS
BEGIN
        DECLARE 
        ...
        SELECT  @opakowanie = ''	DECLARE @return XML '' + char(10),
                @select = ''SELECT @return = ( ''+ char(10) + 
						''		SELECT * FROM (''+ char(10) + 
						''			SELECT h.issuingPersonContractorId ,c.shortName name ,c.code code, 
										sum(CASE va.textValue WHEN ''''internet'''' THEN 1 ELSE 0 END) internet_quantity , sum(CASE va.textValue WHEN ''''internet'''' THEN (h.netValue  * h.exchangeRate)/h.exchangeScale ELSE 0 END) internet_netValue ,
										sum(CASE va.textValue WHEN ''''wholesale'''' THEN 1 ELSE 0 END) wholesale_quantity , sum(CASE va.textValue WHEN ''''wholesale'''' THEN (h.netValue  * h.exchangeRate)/h.exchangeScale ELSE 0 END) wholesale_netValue ,
										sum(CASE va.textValue WHEN ''''other'''' THEN 1 ELSE 0 END) other_quantity , sum(CASE va.textValue WHEN ''''other'''' THEN (h.netValue  * h.exchangeRate)/h.exchangeScale ELSE 0 END) other_netValue ,
										sum(CASE WHEN va.textValue IS NULL THEN 1 ELSE 0 END) null_quantity , sum(CASE WHEN  va.textValue IS NULL  THEN (h.netValue  * h.exchangeRate)/h.exchangeScale ELSE 0 END) null_netValue '',
		...
		SELECT @exec	= @opakowanie + @select + @from + ISNULL( '' WHERE '' + @where ,'''') + '' GROUP BY h.issuingPersonContractorId ,c.shortName  ,c.code ) line ORDER BY name FOR XML AUTO);
