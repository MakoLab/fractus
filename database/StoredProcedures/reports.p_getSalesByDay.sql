/*
name=[reports].[p_getSalesByDay]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
Y0tL1RClDqFTi+Pj6vtl4w==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getSalesByDay]') AND type in (N'P', N'PC'))
DROP PROCEDURE [reports].[p_getSalesByDay]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getSalesByDay]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'

CREATE PROCEDURE [reports].[p_getSalesByDay]
@xmlVar XML
AS
BEGIN
        DECLARE 
	   ...
	   SELECT @condition = (
        ...
        SELECT  @opakowanie = ''	DECLARE @return XML '',
                @select = ''SELECT @return = (  SELECT convert(varchar(10),[date], 21) [date], quantity,cost, netValue, grossValue, (netValue - cost) profit, (netValue - cost) / NULLIF(netValue,0) profitMargin FROM (
						SELECT date, quantity,cost, netValue, grossValue, (netValue - cost) profit, (netValue - cost) / NULLIF(netValue,0) profitMargin
						FROM (
							SELECT convert(varchar(10),[dbo].[f_reportsDateSelector](issueDate,eventDate,dt.documentCategory), 21) date , 
									...
									--(sum((l.netValue * CommercialDocumentHeader.exchangeRate)/CommercialDocumentHeader.exchangeScale) - sum(ISNULL(cv.value,0) * SIGN(l.quantity ))) profit , 
									--(sum((l.netValue * CommercialDocumentHeader.exchangeRate)/CommercialDocumentHeader.exchangeScale) - sum(ISNULL(cv.value,0)* SIGN(l.quantity ))) / NULLIF(sum( (l.netValue * CommercialDocumentHeader.exchangeRate )/CommercialDocumentHeader.exchangeScale),0) profitMargin
									'',
		...
		SELECT @exec	= @opakowanie + @select + @from + ISNULL( '' WHERE '' + @where ,'''') + ISNULL(@condition,'''') + '' 
			GROUP BY convert(varchar(10), [dbo].[f_reportsDateSelector](issueDate,eventDate,dt.documentCategory), 21)  
			 ) x ) line  ORDER BY [date] DESC  FOR XML AUTO);
