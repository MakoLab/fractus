/*
name=[reports].[p_getPurchaseByItemGroup]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
MaD1o36T0kxJyEmZxCk1NA==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getPurchaseByItemGroup]') AND type in (N'P', N'PC'))
DROP PROCEDURE [reports].[p_getPurchaseByItemGroup]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getPurchaseByItemGroup]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE [reports].[p_getPurchaseByItemGroup]
@xmlVar XML
AS
BEGIN
        DECLARE 
		...
		SELECT @exec	= @opakowanie + @select + @from + ISNULL( '' WHERE '' + @where ,'''') + '' GROUP BY im.itemGroupId  ) line ORDER BY itemGroupId  FOR XML AUTO);
