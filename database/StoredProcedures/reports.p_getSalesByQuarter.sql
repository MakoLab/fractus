/*
name=[reports].[p_getSalesByQuarter]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
9kzUbr67YdbyaJXrl6JkgA==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getSalesByQuarter]') AND type in (N'P', N'PC'))
DROP PROCEDURE [reports].[p_getSalesByQuarter]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getSalesByQuarter]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE [reports].[p_getSalesByQuarter]
@xmlVar XML
AS
BEGIN
        DECLARE 
        ...
        SELECT  @opakowanie = ''	DECLARE @return XML '',
                ...
                SELECT quarter, quantity,cost, netValue, grossValue, (netValue - cost) profit, (netValue - cost) / NULLIF(netValue,0) profitMargin
						FROM (
							SELECT left(convert(varchar(10), CommercialDocumentHeader.eventDate, 21),4) + cast(datepart(qq,CommercialDocumentHeader.eventDate) as varchar(50)) quarter   
		...
		SELECT @exec	= @opakowanie + @select + @from + ISNULL( '' WHERE '' + @where ,'''') + ISNULL(@condition ,'''') + '' GROUP BY  left(convert(varchar(10), CommercialDocumentHeader.eventDate, 21),4) + cast(datepart(qq,CommercialDocumentHeader.eventDate) as varchar(50)) ) x  ) line ORDER BY quarter FOR XML AUTO);
