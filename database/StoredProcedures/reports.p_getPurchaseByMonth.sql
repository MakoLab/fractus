/*
name=[reports].[p_getPurchaseByMonth]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
noqlizpSXBgJvHnKQskzDw==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getPurchaseByMonth]') AND type in (N'P', N'PC'))
DROP PROCEDURE [reports].[p_getPurchaseByMonth]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[reports].[p_getPurchaseByMonth]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'
CREATE PROCEDURE [reports].[p_getPurchaseByMonth]
@xmlVar XML
AS
BEGIN
        DECLARE 
		...
		SELECT @exec	= @opakowanie + @select + @from + ISNULL( '' WHERE '' + @where ,'''') + '' GROUP BY  left(convert(varchar(10), CommercialDocumentHeader.eventDate, 21),4) + ''''-'''' + substring(convert(varchar(10), CommercialDocumentHeader.eventDate, 21),6,2)   ) line ORDER  BY month DESC FOR XML AUTO);
