/*
name=[communication].[p_createIncomeOutcomeRelationPackage]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
Lrf25hfHo3CMsYpxtNQFgw==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[communication].[p_createIncomeOutcomeRelationPackage]') AND type in (N'P', N'PC'))
DROP PROCEDURE [communication].[p_createIncomeOutcomeRelationPackage]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[communication].[p_createIncomeOutcomeRelationPackage]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [communication].[p_createIncomeOutcomeRelationPackage]
@xmlVar XML
AS
BEGIN
								...
								( SELECT 
									( SELECT    x.value(''@previousVersion'',''char(36)'') AS ''@previousVersion'',
                                      ...
                                      FROM      @xmlVar.nodes(''root/entry'') AS a ( x )
                                                LEFT JOIN document.IncomeOutcomeRelation i ON i.id = x.value(''@id'', ''char(36)'')
