/*
name=[tools].[p_commercialDocumentDictionaryRebuild]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
jqIcw0vNdh0Mm6Ikj7S8bg==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[tools].[p_commercialDocumentDictionaryRebuild]') AND type in (N'P', N'PC'))
DROP PROCEDURE [tools].[p_commercialDocumentDictionaryRebuild]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[tools].[p_commercialDocumentDictionaryRebuild]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N'CREATE PROCEDURE [tools].[p_commercialDocumentDictionaryRebuild]
