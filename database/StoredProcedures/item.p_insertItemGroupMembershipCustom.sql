/*
name=[item].[p_insertItemGroupMembershipCustom]
version=1.0.1
lastUpdate=2017-01-24 10:37:21
gl/FNQdBWtsW2J5YxP74jw==
*/
IF  EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[item].[p_insertItemGroupMembershipCustom]') AND type in (N'P', N'PC'))
DROP PROCEDURE [item].[p_insertItemGroupMembershipCustom]
GO
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
IF NOT EXISTS (SELECT * FROM sys.objects WHERE object_id = OBJECT_ID(N'[item].[p_insertItemGroupMembershipCustom]') AND type in (N'P', N'PC'))
BEGIN
EXEC dbo.sp_executesql @statement = N' 

CREATE PROCEDURE [item].[p_insertItemGroupMembershipCustom]
@xmlVar XML
AS
...
--return 0;   
	...
	IF ISNULL((select textValue from configuration.Configuration where [key] like ''items.allowOneGroupMembership''),''false'') = ''true''
		BEGIN
			...
			WHERE cg.id IS NOT NULL	AND cg.itemId IS NOT NULL
